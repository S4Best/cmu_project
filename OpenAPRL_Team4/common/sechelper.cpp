...
int server_cipher_init()
{
    ...
    result = encryptAllData(ServerKeyFile, &mBlob);
    if (result < 0)
    {
    ...
    result = encryptAllData(AccountDBKeyFile, &mBlob);
    if (result < 0)
    {
        printf("failed to encrypt DBKey files\n");
        return -1;
    }
    result = encryptAllData(OtpDBKeyFile, &mBlob);
    if (result < 0)
    {
        printf("failed to encrypt DBKey files\n");
        return -1;
    }
    result = encryptAllData(PlateDBKeyFile, &mBlob);
    if (result < 0)
    {
        printf("failed to encrypt DBKey files\n");
        return -1;
...
int client_cipher_init()
{
    ...
    result = encryptAllData(ClientKeyFile, &mBlob);
    if (result < 0) 
    {
        printf("failed to encrypt privateKey\n");
        return -1;
