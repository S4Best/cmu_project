...
static bool detectandshow(Alpr* alpr, cv::Mat frame, std::string region, bool writeJson, int frameno);
...
LGDemoRun::LGDemoRun() :
    stoppable(FALSE), started(FALSE)
    , demoMode({ Mode::mPlayback_Video , VideoSaveMode::vSave, VideoResolution::rNone })
    , licSendFuncPtr(NULL), validStateCheckFuncPtr(NULL)
    , informFuncPtr(NULL), pushRecognizedFuncPtr(NULL)
{
    dmrPtr = this;
}

LGDemoRun::LGDemoRun(lic_send_func lsf, is_valid_state_check_func vcf) :
    stoppable(FALSE), started(FALSE)
    , demoMode({ Mode::mPlayback_Video , VideoSaveMode::vSave, VideoResolution::rNone })
    , licSendFuncPtr(lsf), validStateCheckFuncPtr(vcf)
    , informFuncPtr(NULL), pushRecognizedFuncPtr(NULL)
{
    dmrPtr = this;
}
...
bool LGDemoRun::isNetValid()
{
    if (this->validStateCheckFuncPtr)
    {
        if (this->validStateCheckFuncPtr())
            return TRUE;
    }
...
void LGDemoRun::sendLicenseNumber(char* lic_num, size_t lic_num_len, int frameno)
{

    if (!this->isNetValid())
        return;

    //MSG(mtype::mMsgWithConsole, "sock valid");

    if (this->licSendFuncPtr)
        this->licSendFuncPtr(lic_num, lic_num_len, frameno);
}

void LGDemoRun::run(struct demoMode_s dm)
{
    this->demoMode = dm;
    MSG(mtype::mMsgWithConsole, "video mode(1:live/2:playbock/3:image) : " + std::to_string((long)dm.vMode));
    MSG(mtype::mMsgWithConsole, "video save mode(1:noSave/2:save/3:saveWithNoALPR) : " + std::to_string((long)dm.vSaveMode));
    MSG(mtype::mMsgWithConsole, "res(0:rNone/1:r640X480/2:r1280X720) : " + std::to_string((long)dm.vRes));
...
void LGDemoRun::detachThread()
{
    run_thread.detach();
}

bool LGDemoRun::isStoppable()
{
    bool stop_flag = FALSE;
    mtx.lock();
    stop_flag = this->stoppable;
    mtx.unlock();

    return stop_flag;
}

bool LGDemoRun::isStarted()
{
    bool started_flag = FALSE;
    mtx.lock();
    started_flag = this->started;
    mtx.unlock();

    return started_flag;
}

void LGDemoRun::setStarted(bool started)
{
    mtx.lock();
...
Mode LGDemoRun::getMode()
{
    Mode md;
    mtx.lock();
    md = this->demoMode.vMode;
    mtx.unlock();

    return md;
}

VideoSaveMode LGDemoRun::getSaveMode()
{
    VideoSaveMode vsm;
    mtx.lock();
    vsm = this->demoMode.vSaveMode;
    mtx.unlock();

    return vsm;
}

VideoResolution LGDemoRun::getRes()
{
    VideoResolution res;
    mtx.lock();
    res = this->demoMode.vRes;
    mtx.unlock();

    return res;
}

void LGDemoRun::inform(info_type_e info_t, char* buf, size_t buf_len)
{
    if (this->informFuncPtr)
    {
        this->informFuncPtr(info_t, buf, buf_len);
    }
}

void LGDemoRun::pushFrame(cv::Mat frame, std::string license_plate_num, int frameno)
{
    if (this->pushRecognizedFuncPtr)
    {
        this->pushRecognizedFuncPtr(frame, license_plate_num, frameno);

        this->sendLicenseNumber((char*)license_plate_num.c_str(),
            strlen(license_plate_num.c_str()), frameno);
    }
}

int start_lgdemo()
{
    MSG(mtype::mMsgWithConsole, "start_lgdemo");
    ...
    {
        if (GetFileName(mode, filename)) MSG(mtype::mMsgWithConsole, "Filename is " + std::string(filename));
        else r = LGC_FAILURE;
    }
    ...
    {
        if (dmrPtr)
            videosavemode = dmrPtr->getSaveMode();
        else
    ...
    if (mode == Mode::mLive_Video)
    {
        ...
        if (!cap.isOpened()) {
            MSG(mtype::mMsgWithConsole, "Error opening video stream");
            goto lgdr_exit;
        }
    }
    else if (mode == Mode::mPlayback_Video)
    {
        cap.open(filename);
        if (!cap.isOpened()) {
            MSG(mtype::mMsgWithConsole, "Error opening video file");
            goto lgdr_exit;
        }
    }
    ...
    if (mode != Mode::mImage_File)
    {
        ...
        MSG(mtype::mMsgWithConsole, "Frame width= %d height=%d\n", frame_width, frame_height);
        ...
        if (videosavemode != VideoSaveMode::vNoSave)
        {
            ...
            {
                MSG(mtype::mMsgWithConsole, "Could not open the output video for write");
                goto lgdr_exit;
            }
        }
    }

    while (!dmrPtr->isStoppable()) {
        ...
        {
            detectandshow(&alpr, frame, "", false, frameno);
#if 0
            //TODO send license to server
            GetResponses();
#endif
        ...
        sprintf_s(text, "avg time per frame %f ms. jitter %f ms. fps %f. frameno = %d", avgdur(dur), dur - avgdur(dur), 
            avgfps(), frameno++);
    }
...
lgdr_exit:
    MSG(mtype::mMsgWithConsole, "end of lgdemo run");
    if(!dmrPtr->isStoppable())
        dmrPtr->detachThread();

    dmrPtr->inform(INFO_T_END, NULL, 0);
    dmrPtr->setStarted(FALSE);
    return 0;
}
/***********************************************************************************/
/* End Main                                                                        */
        ...
        for (int i = 0; i < results.plates.size(); i++)
        {
...
#endif
            {
                ...
                if (!found)
                {
                    ...
                    if (dmrPtr)
                    {
                        //std::cout << "frameno : " << frameno << ", best match lic num : " << results.plates[i].bestPlate.characters << std::endl;
                        dmrPtr->pushFrame(frame, results.plates[i].bestPlate.characters, frameno);
                    }
#else
